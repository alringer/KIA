...
$(document).ready(function () {
  window.login = {
    ...
    methods: {
      ...
      _toggle_show_password: function _toggle_show_password() {
        var $this = $(this),
            $input = $this.prev('input'); // If it's a password, swap for text, and vice versa
      ...
      _reset_email: function _reset_email(e) {
        e.preventDefault(); // Work from the active modal

        var $active_modal = window.modals.elements.modal.filter('.active'),
            modal_width = $active_modal.outerWidth(); // Progress the password reset to the next slide

        window.login.elements.reset_password_flow.css('left', -modal_width); // Update the UI ( this class hides various UI elements as per design )
      ...
      _back_to_signin: function _back_to_signin() {
        ...
        var $active_modal = window.modals.elements.modal.filter('.active'),
            $errors = $active_modal.find('.error'); // Remove All error styles

        $errors.removeClass('error');
        $errors.find('.error-message').removeAttr('style'); // Reset Classes

        $active_modal.removeClass('reset-flow-finished'); // This class is used to correct some animations and is removed at the end.

        $active_modal.addClass('back-to-sign-in'); // Move back to the first slide

        window.modals.methods._nav_to(1); // Close the modal overlay ( modal overlays cover the content of a modal -- not to be confused with the transparent background )


        window.modals.methods._overlay_toggle('close'); // Reset the password flow overlay


        window.login.elements.reset_password_flow.css('left', 0); // Set all values to what was used as a reset

        var value = window.login.elements.reset_email.val();
        window.login.elements.login_email.val(value);
        window.login.elements.create_email.val(value); // Remove the class used to correct animations
  ...
  window.login.elements.back_to_signin.on('click', window.login.methods._back_to_signin); // DEMO EVENTS
