...
$(document).ready(function () {
  window.forms = {
    ...
    methods: {
      ...
      _error: function _error($field) {
        $field.addClass('error'); // Get the error fields

        var $error_message = $field.is('form') ? $field.children('.error-message') : $field.find('.error-message'),
            $error_message_inner = $error_message.find('.error-message-inner'); // Use css3 transition to slide down error message

        $error_message.css({
          'height': "".concat($error_message_inner[0].clientHeight, "px")
        });
        setTimeout(function () {
          $error_message.css({
            'height': 'auto'
          });
        }, 250); // Toggles opacity, etc.
      ...
      _dual_inputs: function _dual_inputs() {
        var $this = $(this),
            ...
            $radio = $inputs.filter('[type="checkbox"], [type="radio"]'); // Make sure the radio/checkbox gets checked

        $("[name=\"".concat($radio.attr('name'), "\"]")).prop('checked', false);
        $radio.prop('checked', true); // Trigger the default change event
  ...
  window.forms.elements.dual_inputs.on('focus', window.forms.methods._dual_inputs); // RUN METHODS

  window.forms.methods._select_picker(); // window.forms.methods._selectize();
