...
$(document).ready(function () {
    ...
    methods: {
      ...
      _carousel_slide: function _carousel_slide(data) {
        var $this = $(this),
            ...
            $back = $modal.find('.back'); // If we are not inside of a modal, exit.

        if (!$modal.length || !$modal.hasClass('in')) {
          return;
        }

        $carousel_inner.removeClass('overflow-visible'); // Update the UI to hide the nav
        ...
        $nav_items.removeClass('active').eq(slide_index).addClass('active'); // Use CSS to animate the height of the slider while transitioning
  ...
  $('[data-go-to-slide]').on('click', window.modals.methods._go_to_slide); // window.modals.elements.body.on('touchmove', window.modals.methods._stop_scroll);
